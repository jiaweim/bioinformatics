...
隐藏的文件或目录以 `.` 开头，`ls` 默认不显示。这些文件通常为配置文件或目录。使用 `-a` 选项可查看。

- `-A`, `--almost-all`

与 `-a` 类似，但不列出 `.` (当前工作目录) 和 `..`（父目录）。
...
`ls` 默认列出目录中的内容而非目录本身。将此选项与 `-l` 结合，可查看目录的详细信息，而不是其中的内容。
...
在长格式的输出结果中，不再以字节为单位，而是以人类易读形式显示文件大小。

使用 1024 为幂；`M` 表示 1,048,576 字节。该选项等价于 `--block-size=human-readable`。如果希望以 1000 为幂，使用 `--si` 选项。

- `-i`, `--inode`

在文件名左侧输出索引节点号（inode）。此编号为文档系统每个文件的唯一识别号。
