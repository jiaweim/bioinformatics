...
| `/boot` | 包含 Linux 内核、初始化 RAM 磁盘镜像（供引导期所需的驱动程序使用）和启动加载程序。值得注意的文件包括配置引导装载器的 `/boot/grub/grub.conf` (或 menu.lst) 和 Linux 内核的 `/boot/vmlinuz` |
| `/dev` | 包含系统上设备的访问点文档，包括终端设备 `tty*`，硬盘（`hd*` 或 `sd*`），RAM（`ram*`）和 CD-ROMs（`cd*`）。用户可以直接通过这些设备文档访问设备，不过，应用程序对终端用户通常会隐藏实际的设备名称 |
| `/etc` | 包含所有系统层面的配置文件，以及一组用于在引导期间启动各个系统服务的 Shell 脚本。这些文档大多是纯文本文件。如 `/etc/crontab` 文件定义了何时执行自动化作业；`/etc/fstab` 文件指定了存储设备及其关联的挂载点；`/etc/passwd` 文件包含系统中所有用户信息 |
| `/home` | 用户主目录，正常配置中每个用户在 `/home` 中都有各自的目录。普通用户仅对自己主目录中的文件有写权限 |
| `/lib` | 包含 `/bin` 和 `/sbin` 中核心系统程序启动所需的共享库 |
| `/loss+found` |每个采用 Linux 文件系统（如 ext3） 格式化过的分区都会包含该姆露露。它用于文件系统损坏时的部分恢复。除非系统发生严重问题，否则这个目录总是空的|
| `/media` | 包含可移动介质的挂载点，例如 USB 驱动器，CD-ROMs 等等，这些介质连接到计算机后，会自动挂载到这个目录下 |
| `/mnt` | 早期 Linux 系统中，该目录包含可移动介质的挂载点 |
| `/opt` | 用来安装可选软件。主要用来存储安装的商业软件 |
| `/proc` | 该目录比较特殊，它并非存储于硬盘上的真实文件系统，而是由 Linux 内核维护的虚拟文件系统。该目录包含的文件都是可读的，包含系统资源信息 |
| `/root` | root 账户的 home 目录，出于安全考虑，root 的 home 目录不在 `/home` 目录下 |
| `/sbin` | 包含系统可执行二进制文件，负责执行重要的系统任务，通常为超级用户保留 |
| `/tmp` | 保存各种程序生成的临时文件。有些配置会使该目录在每次系统重启时被清空 |
| `/usr` | `/usr` 可能是 Linux 系统中最大的目录树，包含普通用户所需要的所有程序和文件 |
| `/usr/bin` | 包含系统安装的可执行程序，这个目录会包含许多程序 |
| `/usr/lib` | `/usr/bin` 中的程序需要的共享库 |
| `/usr/local` | 包含非 Linux 发行版自带的程序，但计划在系统范围内使用。通常，由源码编译的程序会安装在 `/usr/local/bin` 目录。新安装的 Linux 系统中会存在这个目录，并且在管理员安装程序之前，这个目录是空的 |
| `/usr/sbin` | 包含许多系统管理程序 |
| `/usr/share` | 包含 `/usr/bin` 中的程序需要的共享数据。包括默认配置文件、图标、桌面背景、音频文件等等 |
| `/usr/share/doc` | 大多数安装在系统中的软件会自带文档，在该目录可以找到按照软件包分类的文档 |
| `/var` | 除 `/tmp` 和 `/home` 目录之外，到目前为止看到的目录都是相对静止的，即不怎么发生变化。那些可能会改变的数据保存在 `/var` 目录树种。各种数据库、假脱机文件、用户邮件等都在其中|
| `/var/log` | 包含日志文件、各种系统活动记录。这些文件非常重要，应该被随时监控。其中较有用的是 `/var/log/messages` 和 `/var/log/syslog`。出于安全考虑，在有些系统中，只有超级用户才能查看日志文件|
...
| * | 所有文件 |
| g* | 以 g 开头的文件 |
| b*.txt | 以 b 开头，以 `.txt` 结尾的文件 |
| Data??? | 以 Data 开头，并紧跟 3 个字符的文件 |
| [abc]* | 以 a, b, 或 c 开头的文件 |
| BACKUP.[0-9][0-9][0-9] | 以 BACKUP. 开头并紧跟 3 个数字的文件 |
| `[[:upper:]]*` | 以大写字母开头的文件 |
| `[![:digit:]]*` | 不以数字开头的文件 |
| `*[[:lower:]123]` | 以小写字母或 1，2，3 任意数字结尾的文件 |
